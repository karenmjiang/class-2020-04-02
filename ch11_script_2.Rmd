...
poverty_model %>%
  tidy(conf.int = TRUE)
```
The intercept means that when everyone has a high school eduction (percent with less_than_hs is 0), the poverty rate is 6.64%.

As the percentage of individuals with less than a high school education increases by one percent, the rate of poverty increases by 0.64%
...
```{r}
x <- county %>%
  rep_sample_n(rep = 1000, size = nrow(county), replace = TRUE) %>%
  group_by(replicate) %>%
  nest()
```
...
```{r}
lm(poverty ~ less_than_hs, data = county) %>%
  tidy(conf.int = TRUE) %>%
...
```{r}
prediction <- 10.9 * results$estimate[2] + results$estimate[1]
...
summary <- poverty_model %>%
  augment() %>%
  select(poverty, less_than_hs, .fitted, .resid)

summary %>%
  head()
```
.fitted = results on the line
.resid = actual results - predicted / fitted results
positive residual is that the actual value is higher than the predicted / fitted.
...
poverty_model %>% 
  augment() %>%
  mutate(
    conf_low = .fitted - .se.fit*(1.96),
    conf_high = .fitted + .se.fit*(1.96)
  ) %>%
  select(poverty, less_than_hs, conf_low, .fitted, conf_high)
...
babynames %>%
  ...
  labs(
    title = "Popularity of American names in the previous 140 years",
